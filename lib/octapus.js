...
class Router{
    ...
    #mergeNode(currentNode,nodeToMerge){
        ...
        for(const [pathSegment,subNode] of Object.entries[nodeToMerge.children]){
            ...
            this.#mergeNode(currentNode.children[pathSegment],subNode);
        }
        ...
        if(nodeToMerge.param){
            ...
            this.#mergeNode(currentNode.param,nodeToMerge.param);
        }
    }
    ...
    #printNode(node,prefix,level=0,prefixSymbol=""){
            ...
            for(const [method,handler] of Object.entries(node.handler)){
                const handlerName=handler.name|| handler.toString().replace(/[\n]/g,"").replace(/[\s]{2,}/g," ").substring(0,30)+"...";
        ...
        async handleRequest(nativeReq,nativeRes){
            ...
            if(!routeHandler){
                ...
                nativeRes.send("Route not found");
                return;
            }
            ...
            const routeHandlerFunc=routeHandler[routePath] || routeHandler.requestHandler;
        ...
        merge(routeToMerge){
            this.#mergeNode(this.rootNode,routeToMerge.rootNode)
        ...
        #nestNodes(){
            ...
            this.#mergeNode(currentNode,newRouter.rootNode);
        }
...
class Request{
    ...
    get method(){
        return tyhis.#nativeRequest.method;
    }
    get url(){
...
class Response{
    ...
    /**
     * Response status() checks for content-type/header
     */
    status(statusCode,headers){
        this.#nativeResponse.status(statusCode,headers)
...
function createServer(router){
    return hrrp.createServer((req,res)=>{
