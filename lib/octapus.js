...
class Response {
    ...
    // function to response static files
    send(){
...
function createServer(router) {
    ...
    nodemon.on('start',()=>{
    console.log(`Octapus server started...`)
    }).on('quit',()=>{
        console.log(`Octapus server stopped...`)
        process.exit();
    }).on('restart',(files)=>{
        console.log(`Octapus server restarting due to changes in ${files}`);
    })
