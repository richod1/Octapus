...
class MongoWrapper{
    ...
    async updateMany(collectionName,filter,update,options={}){
        try{
            ...
            const result=await this.db.collection(collectionName).updateMany(filter,update,options);
            return result.modifiedCount;
    ...
    async replaceOne(collectionName,filter,replacement,options={}){
        try{
            const result=await this.db.collection(collectionName).replaceOne(filter,replacement,options);
            return result.modifiedCount;

        }catch(err){
            console.error(`Error replacing ${collectionName}`,err.message);
            throw err;
        }
    }
    ...
    async deleteOne(collectionName,filter,options={}){
        try{
            const result=await this.db.collection(collectionName).deleteOne(filter,options);
    ...
    async deleteMany(collectionName,filter,options={}){
        ...
        try{
            const result=await this.db.collection(collectionName).deleteMany(filter,options);
            return result.deletedCount;

        }catch(err){
            console.error(`Error deleting many ${collectionName}`,err);
            throw err;
        }
    }
}
